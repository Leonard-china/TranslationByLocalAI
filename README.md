...
- 内置 200 篇高考趋势英语阅读材料，可按来源、主题、难度和长度筛选并完全离线阅读。
- 阅读正文时单击任意英文单词，直接使用随包本地词典瞬时显示中文释义，不调用 AI。
- 正文以语义化 HTML 保存和排版，使用大字号、宽行距、段落与小标题提高长文可读性。
- 点击“浏览器打开”会将当前文章保存为完整的 UTF-8 HTML 文件，并使用系统默认浏览器打开。
- 选中一个句子后可点击“按原模式翻译选中句”，完全沿用软件当前的翻译方向和详细翻译设置。

## 离线英语阅读库

右键系统托盘图标，选择“离线英语阅读库”即可打开。文章库包含 188 篇经署名
过滤的 VOA Learning English 原创文章，以及 12 篇带 Creative Commons 许可的
Nature Portfolio 开放获取摘要精读。支持全文搜索，并可按来源、主题、难度和
篇幅组合筛选。

文章正文以语义化 HTML 片段随软件保存在本地，段落、小标题和列表结构不会在
离线保存时丢失。7,767 个去重英文词形也全部收入本地词典。普通词汇显示 ECDICT
中文释义；人名、缩写和新复合词也有本地识别词条，因此阅读中不会出现需要
联网或等待 AI 的“死点击”。句子翻译不预存结果，用户主动点击后仍走软件原有的
本地或云端翻译模式。“浏览器打开”导出的文章保存在
`%LOCALAPPDATA%\TranslationByLocalAI\Articles`，可直接复制、收藏或离线打开。

完整的来源、许可、筛选口径和分类统计见
[ARTICLE-SOURCES.md](ARTICLE-SOURCES.md)。
...
- 可使用 `TranslationByLocalAI.exe --reader` 启动后直接打开离线阅读库。
